...
<?php
	...
	if($_GET['operation']=="install"){
		...
		$menuClassRow=$empire->fetch1("select classid from {$dbtbpre}enewsmenuclass where classname='{$ecms_charset_config['keydatas_name']}' limit 1");
		if(!$menuClassRow){
			
			$empire->query("insert into `{$dbtbpre}enewsmenuclass` values(NULL,'{$ecms_charset_config['keydatas_name']}','0','0','2','');");
			$menuClassid=$empire->lastid();
			
			$empire->query("insert into `{$dbtbpre}enewsmenu` values(NULL,'{$ecms_charset_config['plugins_name']}','../extend/keydatas/index.php','0','$menuClassid','1');");
			
			$empire->query("insert into `{$dbtbpre}enewsmenu` values(NULL,'{$ecms_charset_config['plugins_setup']}','../extend/keydatas/setup/setup.php','0','$menuClassid','1');");
			}
